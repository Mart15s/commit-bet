...
export default function AuthLayout({ children }: { children: React.ReactNode }) {
  return (
    <main className="grid min-h-screen px-5 py-6 sm:px-8 lg:grid-cols-[1fr_.9fr] lg:gap-10">
      <section className="hidden min-h-[calc(100vh-3rem)] rounded-[1.5rem] bg-[#172019] p-8 text-white lg:flex lg:flex-col lg:justify-between">
        <Link href="/" className="text-xl font-black tracking-[-.03em]">CommitBet</Link>
        <div className="max-w-xl">
          <p className="text-xs font-black uppercase tracking-[.18em] text-[#e9f45b]">Accountability platform</p>
          <h1 className="mt-3 text-5xl font-black leading-[.96] tracking-[-.055em]">
            Finish the project your team promised to finish.
          </h1>
          <p className="mt-5 leading-7 text-[#c5cec6]">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <div key={String(label)} className="rounded-2xl border border-white/12 bg-white/7 p-4">
              <Icon className="text-[#e9f45b]" size={20} />
