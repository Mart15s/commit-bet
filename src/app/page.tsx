  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-5 pb-14 pt-8 sm:px-8 md:grid-cols-[1.05fr_.95fr] md:items-center md:pb-20 md:pt-16">
        <div>
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-[#d8e2a6] bg-[#fbffe4] px-3 py-2 text-xs font-black uppercase tracking-[.12em] text-[#4f5d19]">
          ...
          <div className="mt-5 grid gap-2 text-sm font-bold text-[var(--muted)] sm:grid-cols-3">
            <span className="inline-flex items-center gap-2"><ShieldCheck size={16} className="text-[var(--brand)]" /> Virtual pledges only</span>
            <span className="inline-flex items-center gap-2"><Brain size={16} className="text-[var(--brand)]" /> AI recommends</span>
            <span className="inline-flex items-center gap-2"><Users size={16} className="text-[var(--brand)]" /> Humans confirm</span>
          </div>
        </div>

        <div className="rounded-[1.5rem] border border-[var(--line)] bg-white p-4 shadow-[0_24px_80px_rgba(23,32,25,.12)] sm:p-5">
          <div className="rounded-[1.15rem] border border-[var(--line)] bg-[#f8faf5] p-4">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-black uppercase tracking-[.14em] text-[var(--brand)]">Active sprint</p>
                <h2 className="mt-1 text-2xl font-black tracking-[-.03em]">{sampleProject.title}</h2>
                <p className="mt-2 text-sm leading-6 text-[var(--muted)]">{sampleProject.goal}</p>
              </div>
              <StatusBadge status="active" />
            </div>
            <div className="mt-5 grid grid-cols-3 gap-2">
              <Card className="p-3"><p className="text-xs font-bold text-[var(--muted)]">Score</p><p className="text-2xl font-black">{sampleProject.successScore}</p></Card>
              <Card className="p-3"><p className="text-xs font-bold text-[var(--muted)]">Proof</p><p className="text-2xl font-black">{submitted}</p></Card>
              <Card className="p-3"><p className="text-xs font-bold text-[var(--muted)]">Approved</p><p className="text-2xl font-black">{approved}</p></Card>
            </div>
            <div className="mt-5">
                ...
                <div key={task.id} className="rounded-xl border border-[var(--line)] bg-white p-3">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-[var(--muted)]">{task.ownerName} · expects {task.expectedEvidenceTypes.join(", ")}</p>
                    </div>
      ...
      <section className="border-y border-[var(--line)] bg-white">
        <div className="mx-auto grid max-w-7xl gap-6 px-5 py-12 sm:px-8 md:grid-cols-[.8fr_1.2fr] md:py-16">
          <div>
            <p className="text-xs font-black uppercase tracking-[.16em] text-[var(--brand)]">The problem</p>
      ...
      <section className="mx-auto max-w-7xl px-5 py-14 sm:px-8 md:py-20">
        <div className="max-w-2xl">
          <p className="text-xs font-black uppercase tracking-[.16em] text-[var(--brand)]">How it works</p>
        ...
        <div className="mt-8 grid gap-3 md:grid-cols-3">
          {flow.map(([title, copy], index) => (
            <Card key={title} className="p-5">
              <span className="text-sm font-black text-[var(--brand)]">{String(index + 1).padStart(2, "0")}</span>
      ...
      <section className="bg-[#172019] text-white">
        <div className="mx-auto grid max-w-7xl gap-8 px-5 py-14 sm:px-8 md:grid-cols-[.95fr_1.05fr] md:py-20">
          <div>
            <p className="text-xs font-black uppercase tracking-[.16em] text-[#e9f45b]">Different from Trello, Notion, Jira</p>
            <h2 className="mt-2 text-3xl font-black tracking-[-.04em] sm:text-4xl">CommitBet is for finishing, not organizing forever.</h2>
            <p className="mt-4 leading-7 text-[#c4cec6]">A sprint has success criteria, owners, pledge points, evidence, peer approval, and a final human-confirmed outcome.</p>
          </div>
          <div className="grid gap-3">
            {differences.map(([title, copy]) => (
              <div key={title} className="rounded-2xl border border-white/12 bg-white/7 p-5">
                <h3 className="font-black">{title}</h3>
                <p className="mt-2 text-sm leading-6 text-[#c4cec6]">{copy}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="mx-auto grid max-w-7xl gap-6 px-5 py-14 sm:px-8 md:grid-cols-3 md:py-20">
        <Card className="p-6 md:col-span-2">
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            {["Screenshots", "GitHub links", "Demo videos"].map((item) => <div key={item} className="rounded-xl bg-[#f3f6f1] p-4 text-sm font-black">{item}</div>)}
          </div>
        </Card>
        <Card className="p-6">
          <Gauge className="text-[var(--brand)]" />
          <h3 className="mt-5 text-xl font-black">AI risk insight</h3>
          <p className="mt-2 text-sm leading-6 text-[var(--muted)]">{sampleProject.aiRiskInsight.summary}</p>
          <p className="mt-4 rounded-xl bg-[#fff6d9] p-3 text-sm font-bold text-[#735813]">Why: {sampleProject.aiRiskInsight.why}</p>
        </Card>
      </section>
        ...
        <div className="mx-auto grid max-w-7xl gap-6 px-5 py-14 sm:px-8 md:grid-cols-3 md:py-16">
          {[
            ...
            <div key={String(title)} className="flex gap-4">
              <div className="rounded-xl bg-[#eef3ec] p-3 text-[var(--brand)]"><Icon size={22} /></div>
              <div>
      ...
      <section className="mx-auto max-w-4xl px-5 py-16 text-center sm:px-8 md:py-24">
        <Layers3 className="mx-auto text-[var(--brand)]" size={34} />
        ...
        <p className="mx-auto mt-4 max-w-2xl leading-7 text-[var(--muted)]">Build the project, prove the work, review the evidence, and let humans make the final call.</p>
