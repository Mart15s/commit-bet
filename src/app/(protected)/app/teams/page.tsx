...
import { I18nInput } from "@/components/i18n-fields";
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        <Card>
          ...
          <form action={createTeam} className="mt-4 grid gap-3">
            <label><T k="teams.teamName" /><I18nInput name="name" minLength={2} required placeholderKey="teams.teamNamePlaceholder" /></label>
        ...
        <Card>
          ...
          <form action={joinTeam} className="mt-4 grid gap-3">
            <label><T k="teams.inviteCode" /><I18nInput name="code" required placeholderKey="teams.inviteCodePlaceholder" className="uppercase" /></label>
        ...
        {memberships?.length ? memberships.map((membership) => {
          ...
          return (
            <Card key={team.id}>
              ...
              <p className="mt-2 text-sm text-muted-foreground"><T k="teams.inviteCodeValue" params={{ code: team.invite_code }} /></p>
              <ButtonLink href={`/app/teams/${team.id}`} className="mt-5 w-full" variant="secondary"><T k="teams.openTeam" /></ButtonLink>
            </Card>
          );
        }) : <div className="sm:col-span-2 lg:col-span-3"><EmptyState title={<T k="teams.emptyTitle" />} copy={<T k="teams.emptyCopy" />} /></div>}
      </div>
    </>
  );
}
