...
export default async function TeamPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      <PageHeader
        eyebrow={<T k="teams.detailEyebrow" />}
        title={team.name}
        description={<><T k="teams.inviteCodeLabel" /> {team.invite_code}</>}
        action={<ButtonLink href={`/app/projects/new?team=${team.id}`}><T k="teams.newProject" /></ButtonLink>}
      />
      <Card>
        <h2 className="text-lg font-black"><T k="teams.members" /> ({members?.length ?? 0}/5)</h2>
