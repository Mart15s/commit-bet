...
import { ReviewDecisionSelect } from "@/components/i18n-fields";
...
export default async function DisputePage({
  ...
  return (
    <>
      <ButtonLink href={`/app/tasks/${dispute.task_id}`} variant="ghost" size="sm" className="mb-3"><T k="dispute.back" /></ButtonLink>
      <PageHeader title={<T k="dispute.title" params={{ title: task.title }} />} description={<T k="dispute.description" />} action={<StatusBadge status={dispute.status} />} />
      <ErrorMessage message={query.error} />
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        <Card><h2 className="font-black"><T k="dispute.performerPosition" /></h2><p className="mt-3 text-sm leading-6 text-muted-foreground">{dispute.performer_explanation}</p><h3 className="mt-5 text-sm font-black"><T k="dispute.reviewerRejection" /></h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{dispute.reviewer_rejection_reason}</p></Card>
        <Card className="border-cyan-300/25 bg-cyan-400/10">
          <div className="flex justify-between gap-3"><h2 className="font-black"><T k="dispute.aiRecommendation" /></h2>{recommendation && <strong><T k="dispute.confidence" params={{ score: recommendation.confidence_score }} /></strong>}</div>
      ...
      {isOwner && dispute.status === "open" && <Card className="sticky bottom-16 mt-5 md:bottom-3"><h2 className="font-black"><T k="dispute.humanResolution" /></h2><form action={resolveDispute} className="mt-3 flex flex-col gap-3 sm:flex-row"><input type="hidden" name="dispute_id" value={id} /><ReviewDecisionSelect name="resolution" approvedValue="approve" rejectedValue="reject" /><Button className="shrink-0" type="submit"><T k="dispute.confirmResolution" /></Button></form></Card>}
    </>
  );
}
