...
const initialActionState: DailyLogActionState = { status: "idle" };
...
export function DailyLogForm({
  ...
  const initialProjectId = initialProject
  ...
  const [logDate, setLogDate] = useState(() => {
    const selected = projects.find((item) => item.id === initialProjectId);
    return selected && selected.end_date < today ? selected.end_date : today;
  });
  ...
  const actionError = actionState.status === "error"
    ? actionState.message
    : error;
    ...
    <form action={formAction} className="grid gap-4">
      ...
      <Card className="grid gap-4">
        <div className="grid gap-4 sm:grid-cols-[1.2fr_.8fr]">
          <label>Project<select name="project_id" value={projectId} onChange={(event) => {
            const nextProject = projects.find((item) => item.id === event.target.value);
            setProjectId(event.target.value);
            setSelectedTaskIds([]);
            if (nextProject) {
              setLogDate(nextProject.end_date < today ? nextProject.end_date : today);
            }
          }}>{projects.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label>
          <label>Date<input name="log_date" type="date" min={project?.start_date} max={project?.end_date && project.end_date < today ? project.end_date : today} value={logDate} onChange={(event) => setLogDate(event.target.value)} required /></label>
        </div>
        ...
        <label>
          ...
          <textarea name="summary" required maxLength={4000} value={summary} onChange={(event) => setSummary(event.target.value)} className="min-h-28" placeholder="Example: Finished the evidence submission UI and attached the first mobile screenshots." />
        </label>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <label><span className="inline-flex items-center gap-2"><Clock3 size={16} /> Time spent</span><small className="font-normal text-muted-foreground">Approximate minutes are fine.</small><input name="time_spent_minutes" type="number" min={0} max={1440} value={timeSpentMinutes} onChange={(event) => setTimeSpentMinutes(event.target.value)} required /></label>
          <label><span className="inline-flex items-center gap-2"><Link2 size={16} /> 3. Add proof link</span><small className="font-normal text-muted-foreground">Paste a commit, screenshot, demo, Figma, doc, or video link.</small><input name="proof_link" type="url" maxLength={2048} value={proofLink} onChange={(event) => setProofLink(event.target.value)} placeholder="https://github.com/... or demo link" /></label>
        </div>
        ...
        <label>4. Any blockers?<small className="font-normal text-muted-foreground">Leave blank if nothing is stuck.</small><textarea name="blockers" maxLength={4000} value={blockers} onChange={(event) => setBlockers(event.target.value)} className="min-h-20" placeholder="Example: Waiting on API credentials before I can test the live flow." /></label>
        <label>5. What is next?<small className="font-normal text-muted-foreground">Name the next small move so tomorrow is easy to start.</small><textarea name="next_steps" maxLength={4000} value={nextSteps} onChange={(event) => setNextSteps(event.target.value)} className="min-h-20" required placeholder="Example: Add task evidence from the demo link and ask Sam to review it." /></label>
      </Card>
      <DailyLogSubmitButton
        pending={pending}
        retry={actionState.status === "error"}
      />
    </form>
  );
}
